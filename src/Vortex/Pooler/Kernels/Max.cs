...
{
    public class Max : BasePooler
    {
        public Max(int strideRow = 1, int strideCol = 1, int padSizeRow = 1, int padSizeCol = 1, double padValue = 0.0)
            ...
            for (var j = 0; j < x.Columns; j += StrideCol)
            {
                ...
                for (var k = 0; k < filter.Rows; k++)
                {
                    ...
                    for (var l = 0; l < filter.Columns; l++)
                        if (l + j < x.Columns)
                            if (val <= x[i + k, j + l] * filter[k, l]) val = x[i + k, j + l] * filter[k, l];
                }
