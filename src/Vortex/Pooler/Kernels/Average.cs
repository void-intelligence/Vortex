...
{
    public class Average : BasePooler
    {
        public Average(int strideRow = 1, int strideCol = 1, int padSizeRow = 1, int padSizeCol = 1, double padValue = 0.0)
        ...
        public override Matrix Pool(Matrix x, Matrix filter)
        {
            ...
            for (var j = 0; j < x.Columns; j += StrideCol)
            {
                ...
                val /= filter.Rows * filter.Columns;
                resArray.Add(val);
            }
